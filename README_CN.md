...
- **Qwen-VL 集成**: 内置独立的 Qwen 文本编码节点，剥离了 API 依赖，纯本地运行，并支持多达 **6 张参考图**输入。
...
- **大容量**: 将原本的 3 图输入扩展为 **6 图输入**，适应更复杂的参考图工作流。
...
3. **Qwen 编码节点**:
   - 将加载好的图片连接到 `Qwen Text Encode (6 Images)` 对应的插槽中，用于引导 Qwen-VL 模型生成。
